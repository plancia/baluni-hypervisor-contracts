...
contract AutoRebal {
    ...
    function liquidityOptions(
        address _hypervisor
    ) public view returns (bool, int24 currentTick) {
    ...
    function autoRebalance(
        ...
        if (!token0Limit) {
            ...
            limitUpper =
                (currentTick / hypervisor.tickSpacing()) *
                hypervisor.tickSpacing() -
                hypervisor.tickSpacing();
            if (limitUpper == currentTick)
                limitUpper = limitUpper - hypervisor.tickSpacing();
            ...
            limitLower =
                ...
                hypervisor.tickSpacing() +
                hypervisor.tickSpacing();
            if (limitLower == currentTick)
                limitLower = limitLower + hypervisor.tickSpacing();
