...
describe("Hypervisor", async () => {
  ...
  beforeEach("deploy contracts", async () => {
    ({ token0, token1, token2, factory, router, hypervisorFactory } =
      await loadFixture(hypervisorTestFixture));
    ...
    hypervisor = (await ethers.getContractAt(
      "Hypervisor",
      hypervisorAddress
    )) as Hypervisor;
  ...
  it("deposit with an incorrect proportion will revert", async () => {
    ...
    await token0
      .connect(alice)
      .approve(hypervisor.address, ethers.utils.parseEther("1000000"));
    await token1
      .connect(alice)
      .approve(hypervisor.address, ethers.utils.parseEther("1000000"));
    await token0
      .connect(alice)
      .approve(uniProxy.address, ethers.utils.parseEther("1000000"));
    await token1
      .connect(alice)
      .approve(uniProxy.address, ethers.utils.parseEther("1000000"));
    ...
    ).to.be.revertedWith("Improper ratio");
    await expect(
      uniProxy
        .connect(alice)
        .deposit(
  ...
  it("calculates fees properly & rebalances to limit-only after large swap", async () => {
    ...
    await token0
      .connect(carol)
      .approve(router.address, ethers.utils.parseEther("10000000000"));
    await token1
      .connect(carol)
      .approve(router.address, ethers.utils.parseEther("10000000000"));
  ...
  it("deposit/withdrawal with many users", async () => {
    ...
    await hypervisor
      .connect(user0)
      .withdraw(user0_liq_balance, user0.address, user0.address, [0, 0, 0, 0]);
    await hypervisor
      .connect(user1)
      .withdraw(user1_liq_balance, user1.address, user1.address, [0, 0, 0, 0]);
    await hypervisor
      .connect(user2)
      .withdraw(user2_liq_balance, user2.address, user2.address, [0, 0, 0, 0]);
    await hypervisor
      .connect(user3)
      .withdraw(user3_liq_balance, user3.address, user3.address, [0, 0, 0, 0]);
    await hypervisor
      .connect(user4)
      .withdraw(user4_liq_balance, user4.address, user4.address, [0, 0, 0, 0]);
    ...
    expect(user0token0Amount.sub(tokenAmount).abs()).to.be.lte(
      ethers.utils.parseEther("1")
    );
    expect(user1token0Amount.sub(tokenAmount).abs()).to.be.lte(
      ethers.utils.parseEther("1")
    );
    expect(user2token0Amount.sub(tokenAmount).abs()).to.be.lte(
      ethers.utils.parseEther("1")
    );
    expect(user3token0Amount.sub(tokenAmount).abs()).to.be.lte(
      ethers.utils.parseEther("1")
    );
    expect(user0token1Amount.sub(tokenAmount).abs()).to.be.lte(
      ethers.utils.parseEther("1")
    );
    expect(user1token1Amount.sub(tokenAmount).abs()).to.be.lte(
      ethers.utils.parseEther("1")
    );
    expect(user2token1Amount.sub(tokenAmount).abs()).to.be.lte(
      ethers.utils.parseEther("1")
    );
    expect(user3token1Amount.sub(tokenAmount).abs()).to.be.lte(
      ethers.utils.parseEther("1")
    );
  });
  ...
  it("can withdraw deposited funds without rebalance", async () => {
    ...
    await hypervisor
      .connect(alice)
      .withdraw(alice_liq_balance, alice.address, alice.address, [0, 0, 0, 0]);
    let tokenAmounts = await hypervisor.getTotalAmounts();
  ...
  it("test pullLiquidity, addBaseLiquidity, addLimitLiquidity", async () => {
    ...
    await hypervisor.pullLiquidity(
      await hypervisor.baseLower(),
      await hypervisor.baseUpper(),
      ethers.utils.parseEther("2000"),
      [0, 0]
    );
    await hypervisor.pullLiquidity(
      await hypervisor.limitLower(),
      await hypervisor.limitUpper(),
      ethers.utils.parseEther("2000"),
      [0, 0]
    );
    ...
    expect(aliceToken0.sub(tokenAmount).abs()).to.be.lte(
      ethers.utils.parseEther("1")
    );
    expect(aliceToken1.sub(tokenAmount).abs()).to.be.lte(
      ethers.utils.parseEther("1")
    );
    ...
    ////TODO
    // add base liquidity
    await hypervisor.addLiquidity(
      await hypervisor.baseLower(),
      await hypervisor.baseUpper(),
      await token0.balanceOf(hypervisor.address),
      await token1.balanceOf(hypervisor.address),
      [0, 0]
    );
  ...
  it("auto rebalance; token0 limit", async () => {
    ...
    let autoRebal = await autoRebalFactory.deploy(
      wallet.address,
      wallet.address,
      hypervisor.address
    );
    ...
    let token0InToken1 = tokenAmounts[0].mul(
      Math.pow(1.0001, Number(slot0[1]))
    );
  ...
  it("auto rebalance; should fail when limit token0 bug on contract", async () => {
    ...
    await autoRebal.addLimitLiquidity(
